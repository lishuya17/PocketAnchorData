...
ION_LIST = ['ZN', 'MN', 'CA','MG', 'K', 'CU', 'NI', 'FE', 'SE', 'CD', 'NA', 'CO', 'HG', 'SR', 'CS', 'GA', 'RB', 'IN', \
...
def get_feature(one_info):
    ...
    if elem == 'ION': 
        atom_in_aa = -1 #np.zeros(len(AA_ATOM_LIST)).tolist()
    else:
    ...
    feature = [atom_elem, aa, atom_in_aa, ss_type] + [atom_type, b/30, formal_charge, vdw, protons, geom, valence] ###
    return feature
...
if __name__ == '__main__':
    ...
    if os.path.exists(outdir+'atom_feature.pk'):
    ...
    with open(outdir+'atom_feature.pk', 'wb') as f:
